                ...
                <>
                    ...
                    <div className='flex items-center justify-between my-3 px-2'>
                        <div className='flex items-center gap-x-1'>
                            <div onClick={() => setBgOptionShow((prev) => !prev)} className='w-9 h-9 bg-gradient-to-r from-red via-blue to-green font-gilroySemiBold text-base text-white rounded-md flex items-center justify-center cursor-pointer'>Pp</div>
                            {bgOptionShow &&
                            <>
                            <div onClick={()=>handleRemoveBackground()} className='w-9 h-9 rounded-md cursor-pointer bg-hober_clr'></div>
                                {postbg.map((item, index) => (
                                    <img onClick={()=> handleBackground(index)} className='w-9 h-9 rounded-md cursor-pointer' key={index} src={item} alt="post bg" />
                                ))}
                            </>
                            }
