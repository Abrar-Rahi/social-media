    ...
    return (
        <>
            <div className='flex items-center justify-between bg-white md:px-3 py-8'>
                ...
                <div className='w-14 h-14 md:w-20 md:h-20 rounded-full bg-cyan-100 lg:hidden' style={{ backgroundImage: `url(${userInfo.profilePicture})`, backgroundSize: "cover", backgroundRepeat: "no-repeat", backgroundPosition: "center" }}></div>
                <div className='w-4/6 flex flex-wrap sm:justify-between gap-1 lg:hidden'>
                    {LeftData.map((item,index)=>(
                        <LeftOtherData key={index} data={item} />
                    ))}
                </div>
                <div  className='relative lg:w-2/5 cursor-pointer'>
                    {!viewSbox ?
                    <div onClick={() => setViewSbox(true)} className='flex items-center justify-center lg:justify-normal lg:gap-1  border border-secondary_color w-12 h-12 md:w-16 md:h-16 lg:h-auto lg:w-full lg:py-2 lg:px-4 rounded-full'>
